...
runtime_root="${WAKE_RUNTIME_ROOT:-${project_root}/.wake-runtime}"
...
export WAKE_ENV_READY=1
export WAKE_PROJECT_ROOT="${project_root}"
...
if [[ "$#" -eq 0 ]]; then
  echo "usage: scripts/project-env.sh -- command [args...]" >&2
  exit 64
fi
