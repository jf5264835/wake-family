...
runtime_root="${WAKE_PROJECT_ROOT}/.wake-runtime"
expected_home="${runtime_root}/home"
expected_cache="${runtime_root}/npm-cache"

echo "[wake] validating writable install environment"
...
touch "${HOME}/.wake-write-test" "${expected_cache}/.wake-write-test"
rm -f "${HOME}/.wake-write-test" "${expected_cache}/.wake-write-test"
echo "[wake] environment passed: HOME=${HOME}, cache=${expected_cache}"

lock_file="${runtime_root}/install.lock"
exec 9>"${lock_file}"
if ! flock -n 9; then
  echo "Another dependency install is already running for ${WAKE_PROJECT_ROOT}." >&2
  exit 75
fi
...
for process in /proc/[0-9]*; do
  ...
  [[ "${process_cwd}" == "${WAKE_PROJECT_ROOT}" ]] || continue
  process_command="$(tr '\0' ' ' <"${process}/cmdline" 2>/dev/null || true)"
  if [[ "${process_command}" == *"npm ci"* ]]; then
    echo "Another npm ci is visible in ${WAKE_PROJECT_ROOT}; refusing to overlap installs." >&2
    exit 75
  fi
done

lockfile_sha256="$(sha256sum "${WAKE_PROJECT_ROOT}/package-lock.json" | awk '{print $1}')"
use_seeded_cache=0
seed_cache="${WAKE_NPM_CACHE_SEED:-}"
if [[ -n "${seed_cache}" && -d "${seed_cache}" ]]; then
  seed_lockfile_sha256="$(cat "${seed_cache}/.wake-lockfile-sha256" 2>/dev/null || true)"
  if [[ "${seed_lockfile_sha256}" == "${lockfile_sha256}" ]]; then
    echo "[wake] restoring image-seeded npm cache"
    cp -a "${seed_cache}/." "${expected_cache}/"
    use_seeded_cache=1
    echo "[wake] image cache seed matched; registry fallback remains available"
  else
    echo "[wake] image cache seed does not match this lockfile; using the network path"
  fi
fi

locked_vinext_output="$({ node --input-type=module - "${WAKE_PROJECT_ROOT}/package-lock.json" <<'NODE'
...
NODE
  ...
  echo "[wake] downloading the complete locked vinext tarball"
  curl \
    --fail \
    --location \
    --silent \
  ...
  echo "[wake] verifying locked vinext tarball integrity"
...
NODE
  echo "[wake] network and integrity preflight passed"
fi

echo "[wake] running exactly one bounded npm ci"
...
timeout \
  --signal=TERM \
  --kill-after="${WAKE_INSTALL_KILL_AFTER:-15s}" \
  "${WAKE_INSTALL_TIMEOUT:-8m}" \
  npm "${npm_ci_args[@]}"
...
node --input-type=module - "${WAKE_PROJECT_ROOT}/node_modules/.wake-install.json" "${lockfile_sha256}" <<'NODE'
...
NODE
echo "[wake] npm ci passed and vinext is available"
